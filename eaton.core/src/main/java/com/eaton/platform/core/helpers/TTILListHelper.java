				...
				if(null != manualPath) {
					...
					while(items.hasNext()) {
						...
						ValueMap properties = item.getValueMap();
							...
							listTypeBean.setWindowFixed(StringUtils.equals(CommonUtil.getStringProperty(properties, CommonConstants.NEW_WINDOW_MANUAL), CommonConstants.TRUE) ? CommonConstants.TARGET_BLANK : StringUtils.EMPTY);
