...
/**
 * <html> Description: This is a Sling Model for Featured List Views.</html> .
 *
 * @author TCS
 * @version 1.0
 * @since 2017
...
public class TtilListFeatureListModel {
	...
	public String getImagePath() {
		...
		if(null == imagePath) {
			if(null != this.linkDestination && StringUtils.startsWith(this.linkDestination, CommonConstants.CONTENT_ROOT_FOLDER)) {
				Resource linkResource = this.resourceResolver.getResource(this.linkDestination);
				if(null != linkResource) {
					Resource jcrResource = linkResource.getChild(CommonConstants.JCR_CONTENT_STR);
					if(null != jcrResource)
					imagePath = CommonUtil.getStringProperty(jcrResource.getValueMap(), CommonConstants.TEASER_IMAGE_PATH);
				}
			} else {
